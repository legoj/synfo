...
Module Module1
    ...
    Function DumpObjects() As String
        ...
        For Each s As String In app.Keys
            ...
            If rapp.Has(s) Then
                rapp.GetObj(s).WriteXML("reg_info", w)
            End If
            If app.HasPatches(s) Then
                ...
                For Each z As String In patches.Keys
                    ...
                    If rapp.Has(dName) Then
                        rapp.GetObj(dName).WriteXML("reg_info", w)
                    End If
                    w.WriteEndElement()
                Next
                w.WriteEndElement()
        ...
        w.WriteStartElement("hotfixes")
        w.WriteAttributeString("count", qfe.Count)
        For Each s As String In qfe.Keys
            qfe.GetObj(s).WriteXML("qfe", w)
        Next
        w.WriteEndElement()
...
Class WMIQFEs
    ...
    Public Overloads Function Read() As Boolean
        ...
        Read = MyBase.Read("SELECT * FROM Win32_QuickFixEngineering", "HotfixID", PROP)
    End Function
End Class
Class WMIProducts
    Inherits SysObjects
            ...
            If x.HasChildNodes Then
                If x.Name.Equals("patches") Then
                    For Each xp As XmlElement In x.ChildNodes
                        Dim p = New SynProps("patch", "reg_info")
                        p.Parse(xp)
